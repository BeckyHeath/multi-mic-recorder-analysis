...
sweep_dir_path = "Data\Field_Sweep_Data\";
...
% Seperate spectral csvs into Groups and Conditions
sweep_dir_path = "Data\Field_Sweep_Data\";
files_wp = dir(sweep_dir_path + AnalysisGroup + "_*early*.csv"); % Early Deployment Signal
files_no_wp = dir(sweep_dir_path + AnalysisGroup + "_*late*.csv"); % Late Deployment Signal 
...
legend('Pre-Deployment', 'Post-Deployment')
grid on
...
% GENERATE SPECTRA 
early_path = audio_dir_path + AnalysisGroup +  "_early_02.wav"; % set example for 
late_path = audio_dir_path + AnalysisGroup + "_late_01.wav";

% Set Sample Size + Load Files 
samples = [13.5*16000,21*16000]; % Sweeps all occur within first 25 seconds

[early_all,Fs] = audioread(early_path,samples);
[late_all,Fs] = audioread(late_path,samples);
...
subplot(2,2,2);
spectrogram(early1,1280,1200,1280,Fs,'yaxis')
title('early')

subplot(2,2,4);
spectrogram(late1,1280,1200,1280,Fs,'yaxis')
title('late')
...
%    
    ...
    for file_no = 1:size(x,2)
        ...
        % We only want to analyse one minute! 
        samples = [1,60*16000];   % Change if Fs is not 16000
